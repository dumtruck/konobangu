...
use crate::{
    url::path_to_file_url_segments,
    windows::{is_windows_verbatim_sep, parse_windows_path_prefix},
};
    ...
    pub fn len(&self) -> usize {
        ...
        match *self {
            ...
            VerbatimUNC { server, share } => {
                8 + server.len() + if share.len() > 0 { 1 + share.len() } else { 0 }
            }
            VerbatimDisk { .. } => 6,
            UNC { server, share } => {
                2 + server.len() + if share.len() > 0 { 1 + share.len() } else { 0 }
            }
...
impl Path {
    ...
    pub fn components(&self) -> Components<'_> {
        ...
        Components {
            path: self.as_str(),
            has_physical_root: has_physical_root(self.as_str(), prefix.clone()),
            prefix,
            front: State::Prefix,
            back: State::Body,
        }
            ...
            if is_sep {
                ...
                if !verbatim {
                    component_start += match tail {
                        [b'.'] => 1,
                        [b'.', sep @ _, ..] if is_sep_byte(*sep as char) => 1,
                        _ => 0,
                    };
                }
            }
...
mod tests {
    ...
    use crate::{url::PathToUrlError, Path, PathBuf};
